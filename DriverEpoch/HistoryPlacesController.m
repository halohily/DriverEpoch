...
#import "PlaceCell.h"
@interface HistoryPlacesController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation HistoryPlacesController
...
- (void)setupView
{
    ...
    [table registerClass:[PlaceCell class] forCellReuseIdentifier:@"myCell"];
...
- (void)setupData
{
    ...
    NSDictionary *parameters = @{@"if": @"GetPlaces", @"user_id":user_id};
    ...
    [manager POST:SERVER_ADDRESS parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
          ...
          success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              ...
              if (code.intValue == 1)
              {
                  ...
                  self.tableData = [tempDic objectForKey:@"places_list"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PlaceCell *cell = [self.myTable dequeueReusableCellWithIdentifier:@"myCell"];
    cell.name.text = [self.tableData objectAtIndex:indexPath.row][@"place_name"];
    cell.address.text = [self.tableData objectAtIndex:indexPath.row][@"place_address"];
    cell.time.text = [self.tableData objectAtIndex:indexPath.row][@"add_time"];
    return cell;
}
